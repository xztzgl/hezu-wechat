...
class View extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    // console.log(nextProps, 122331, this.props);
    if (nextProps.val !== this.props.val) {
      this.setState({
        pickerValue: [nextProps.val]
      });
    }
    // if (nextProps.bool) {
    //   this.setState({
      ...
      <div className={styles.inputBody}>
        <div className={styles.inputPos}>
          <div className={styles.inputCss}>
            <label
              style={this.props.sizelabel ? { width: this.props.sizelabel } : {}}
            >
              <span style={this.props.sizelabel ? { letterSpacing: "26px", position: "relative", left: "9px" } : {}}>{this.props.label}</span>
            </label>
            <Picker
              // title="选择地区"
