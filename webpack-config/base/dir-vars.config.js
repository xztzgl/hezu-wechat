...
/**/mE.srcRootDir = path.resolve(mE.staticRootDir, './example-admin-2-single-page'); // 项目业务代码根目录
/**//**/mE.buildDir = path.resolve(mE.srcRootDir, '../web-deploy'); // 存放编译后生成的所有代码、资源（图片、字体等，虽然只是简单的从源目录迁移过来）
/**//**/mE.pagesDir = path.resolve(mE.srcRootDir, './pages'); // 存放各个页面独有的部分，如入口文件、只有该页面使用到的css、模板文件等
/**//**/mE.srcDir = path.resolve(mE.srcRootDir, './src'); // 存放组件源代码等
